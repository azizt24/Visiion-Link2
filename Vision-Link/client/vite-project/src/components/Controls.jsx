...
export default function Controls() {
  ...
  useEffect(() => {
    if (screenSharing && screenTrack && tracks) {
      const fun = async () => {
        try {
          await client.unpublish(tracks[1]);
          await client.publish(screenTrack);
        } catch (error) {
          console.error('Failed to handle screen sharing transition:', error);
        }
      };
      fun();
    }
  ...
  return (
    <div className="controls-page">
      <div className="controllers-video-part">
        ...
        >
          {trackState.audio ? (
            <Tooltip title="Microphone is on" placement="top">
              <MicIcon />
            </Tooltip>
          ) : (
            <Tooltip title="Microphone is off" placement="top">
              <MicOffIcon />
            </Tooltip>
          )}
        </Button>
